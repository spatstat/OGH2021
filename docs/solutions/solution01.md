...
2.  plot the data;
    ...
    By the S3 method dispatch, this calls the `plot.ppp` function. The
    `chars` argument indicate that the point type should be be periods
    (“`.`”).

3.  find out how many trees are recorded;
    
    Both `npoints` and `print.ppp` displays the number of recorded
    trees:
...
4.  find the dimensions of the study region;
    ...
    The dimensions of the observation window can be seen above.
    Alternatively, it can be directly assesed via
...
1.  When you type `plot(ponderosa)`, the command that is actually
    ...
    `main` as is also the case in the regular generic `plot`.
...
2.  plot the Ponderosa data with the title *Ponderosa Pine Trees* above
    ...
    ![](solution01_files/figure-gfm/unnamed-chunk-9-1.png)<!-- -->
...
3.  from your reading of the help file, predict what will happen if we
    ...
    ![](solution01_files/figure-gfm/unnamed-chunk-11-1.png)<!-- -->
...
4.  try different values of the argument `chars`, for example, one of
    ...
    There are subtle differences in the actual character/point types
    plotted. When given a string literal, the actual character is
    plotted as the point type.
    
    ``` r
    plot(ponderosa, chars=3, cols="green")
    ```
    
    ![](solution01_files/figure-gfm/unnamed-chunk-12-1.png)<!-- -->
    
    ``` r
    plot(ponderosa, chars="+", cols="green")
    ```
    
    ![](solution01_files/figure-gfm/unnamed-chunk-12-2.png)<!-- -->
    
    ``` r
    plot(ponderosa, chars=4, cols="green")
    ```
    
    ![](solution01_files/figure-gfm/unnamed-chunk-12-3.png)<!-- -->
    
    ``` r
    plot(ponderosa, chars="X", cols="green")
    ```
    
    ![](solution01_files/figure-gfm/unnamed-chunk-12-4.png)<!-- -->
...
1.  plot the pattern;
    ...
    ![](solution01_files/figure-gfm/unnamed-chunk-13-1.png)<!-- -->
...
2.  plot the pattern again, changing the colours and symbols used to
    ...
    ![](solution01_files/figure-gfm/unnamed-chunk-14-1.png)<!-- -->
...
3.  plot the patterns of pyknotic and dividing cells separately using
    ...
    ![](solution01_files/figure-gfm/unnamed-chunk-15-1.png)<!-- -->
...
Create a point pattern `X` from the data, and plot the point pattern
(use `owin` or `square` to define the study region).
...
With this window, we create the `ppp` object with the funciton of the
same name and plot it:
...
![](solution01_files/figure-gfm/unnamed-chunk-18-1.png)<!-- -->

### Exercise 5

The file `anthills.txt` is available in the Data directory on
[GitHub](https://github.com/spatstat/Melb2018) and downloadable by [this
direct
link](https://raw.githubusercontent.com/spatstat/Melb2018/master/Data/anthills.txt)
...
2.  check the data for any peculiarities.
    ...
    We see that there is a `status` variable indicating that this is a
    *marked* point process.

3.  create a point pattern `hills` containing these data. Ensure that
    the unit of length is given its correct name.
    ...
    The `ppp` function passes the appropriate arguments to `owin` which
    we therefore do not need to call explicitly.
...
4.  plot the data.
    ...
    ![](solution01_files/figure-gfm/unnamed-chunk-22-1.png)<!-- -->
